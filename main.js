...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({ width: 1920, height: 1080, webPreferences: { nodeIntegration: true, partition: 'persist:icloud-drive' } });
  ...
  mainWindow.loadURL('https://www.icloud.com/iclouddrive');
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip('iCloud Drive');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Show iCloud Drive', click: () => { if (!mainWindow) createWindow(); mainWindow.show(); } },
...
app.on('before-quit', async () => { await session.fromPartition('persist:icloud-drive').cookies.flushStore(); });
